...
@router.callback_query(F.data == 'auto-buy')
async def auto_buy_command(callback: types.CallbackQuery, state: FSMContext, db_session):
    """
    Command handler for auto-purchase configuration.
    """
    with db_session as db:
        settings = get_or_create_auto_buy_settings(db, callback.from_user.id)
        user = db.query(User).filter(
            User.user_id == callback.from_user.id).first()
        ...
        await callback.message.answer(
            text=(
                f"<b>{username}! Your balance:</b> {balance} ⭐️\n\n"
                ...
                f"<b>Supply Limit:</b> {settings.supply_limit or 'not set'} pcs.\n"
    ...
    with db_session as db:
        ...
        await message.answer(
            text=(
                ...
                f"<b>Price Limit:</b>\n"
                f"From {settings.price_limit_from} to {settings.price_limit_to} ⭐️\n\n"
                f"<b>Supply Limit:</b> {settings.supply_limit or 'not set'} pcs️.\n"
                f"<b>Purchase Cycles:</b> {settings.cycles}"
            ),
            reply_markup=auto_buy_keyboard(),
            parse_mode="HTML"
